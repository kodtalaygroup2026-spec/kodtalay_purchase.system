...
-- ---------------------------------------------------------------------------
-- 1) ENUM Types — สถานะและบทบาทต่าง ๆ ในระบบ
-- ---------------------------------------------------------------------------

-- บทบาทผู้ใช้ (กำหนดสิทธิ์การเข้าถึง)
create type user_role as enum (
  'admin',      -- ผู้ดูแลระบบ: จัดการได้ทุกอย่าง
  'manager',    -- ผู้อนุมัติ: อนุมัติ PR/PO
  'purchaser',  -- เจ้าหน้าที่จัดซื้อ: ออก PO, ติดต่อ supplier
  'requester',  -- ผู้ขอซื้อ: สร้างใบขอซื้อ (PR)
  'viewer'      -- ผู้ดูข้อมูลอย่างเดียว
);
...
create type pr_status as enum (
  ...
  'approved',   -- อนุมัติแล้ว (พร้อมออก PO)
  'rejected',   -- ไม่อนุมัติ
  'cancelled',  -- ยกเลิก
  'converted'   -- แปลงเป็น PO เรียบร้อยแล้ว
);
...
-- ---------------------------------------------------------------------------
-- 3) ตาราง profiles — เชื่อมกับ auth.users ของ Supabase
--    เก็บข้อมูลเพิ่มเติมของผู้ใช้ เช่น ชื่อ, แผนก, บทบาท
-- ---------------------------------------------------------------------------
...
-- เมื่อมีผู้ใช้สมัครใหม่ใน auth.users ให้สร้าง profile อัตโนมัติ
...
-- ฟังก์ชันตรวจบทบาทของผู้ใช้ปัจจุบัน (ใช้ใน RLS policy)
...
-- ---------------------------------------------------------------------------
-- 4) ตาราง suppliers — ผู้ขาย / คู่ค้า
-- ---------------------------------------------------------------------------
create table public.suppliers (
  id            uuid primary key default gen_random_uuid(),
  code          text unique not null,            -- รหัส supplier เช่น SUP-0001
  name          text not null,
...
-- ---------------------------------------------------------------------------
-- 5) ตาราง categories & products — หมวดหมู่และรายการสินค้า/บริการ
-- ---------------------------------------------------------------------------
...
create table public.products (
  id           uuid primary key default gen_random_uuid(),
  sku          text unique not null,             -- รหัสสินค้า
  name         text not null,
...
create table public.purchase_requisitions (
  id            uuid primary key default gen_random_uuid(),
  pr_number     text unique not null,            -- เลขที่เอกสาร เช่น PR-202606-0001
  title         text not null,
  status        pr_status not null default 'draft',
  requester_id  uuid not null references public.profiles (id),
  department    text,
  needed_by     date,                            -- ต้องการของภายในวันที่
  note          text,
  total_amount  numeric(14, 2) not null default 0, -- ยอดรวม (คำนวณจากรายการย่อย)
  created_at    timestamptz not null default now(),
  updated_at    timestamptz not null default now()
);
...
create index idx_pr_requester on public.purchase_requisitions (requester_id);

create table public.pr_items (
  id          uuid primary key default gen_random_uuid(),
  pr_id       uuid not null references public.purchase_requisitions (id) on delete cascade,
  product_id  uuid references public.products (id) on delete set null,
  description text not null,                      -- เผื่อกรณีสินค้านอกแคตตาล็อก
  quantity    numeric(14, 2) not null check (quantity > 0),
  unit        text not null default 'ชิ้น',
  unit_price  numeric(14, 2) not null default 0 check (unit_price >= 0),
  -- ยอดรวมต่อบรรทัด คำนวณอัตโนมัติ (generated column) เพื่อความถูกต้องเสมอ
  line_total  numeric(14, 2) generated always as (quantity * unit_price) stored,
  created_at  timestamptz not null default now()
);

comment on table public.pr_items is 'รายการย่อยในใบขอซื้อ';
create index idx_pr_items_pr on public.pr_items (pr_id);
...
create table public.purchase_orders (
  id            uuid primary key default gen_random_uuid(),
  po_number     text unique not null,            -- เลขที่เอกสาร เช่น PO-202606-0001
  ...
  vat_rate      numeric(5, 2) not null default 7.00,  -- อัตรา VAT (%)
...
create index idx_po_pr on public.purchase_orders (pr_id);
...
create table public.po_items (
  ...
  quantity     numeric(14, 2) not null check (quantity > 0),
  received_qty numeric(14, 2) not null default 0 check (received_qty >= 0), -- จำนวนที่รับแล้ว
  unit         text not null default 'ชิ้น',
  unit_price   numeric(14, 2) not null default 0 check (unit_price >= 0),
  line_total   numeric(14, 2) generated always as (quantity * unit_price) stored,
  created_at   timestamptz not null default now()
...
-- ---------------------------------------------------------------------------
-- 8) การอนุมัติ (Approvals) — รองรับ workflow หลายขั้น
-- ---------------------------------------------------------------------------
create table public.approvals (
  id            uuid primary key default gen_random_uuid(),
  -- อ้างอิงเอกสารแบบ polymorphic: ระบุชนิดเอกสาร + id
  document_type text not null check (document_type in ('PR', 'PO')),
  document_id   uuid not null,
  approver_id   uuid not null references public.profiles (id),
  step          integer not null default 1,      -- ลำดับขั้นการอนุมัติ
...
create index idx_approvals_document on public.approvals (document_type, document_id);
...
create table public.goods_receipts (
  id           uuid primary key default gen_random_uuid(),
  gr_number    text unique not null,             -- เลขที่เอกสาร เช่น GR-202606-0001
...
create table public.gr_items (
  ...
  quantity    numeric(14, 2) not null check (quantity > 0), -- จำนวนที่รับครั้งนี้
  created_at  timestamptz not null default now()
);

comment on table public.gr_items is 'รายการย่อยในใบรับของ';
create index idx_gr_items_gr on public.gr_items (gr_id);

-- ---------------------------------------------------------------------------
-- 10) Audit log — บันทึกการเปลี่ยนแปลงสำคัญเพื่อตรวจสอบย้อนหลัง
-- ---------------------------------------------------------------------------
create table public.audit_logs (
  id          bigint generated always as identity primary key,
  actor_id    uuid references public.profiles (id),
  action      text not null,                     -- เช่น 'pr.submitted', 'po.approved'
  entity      text not null,                     -- ชื่อตาราง/เอนทิตี
  entity_id   uuid,
  metadata    jsonb,                             -- ข้อมูลเพิ่มเติมแบบยืดหยุ่น
  created_at  timestamptz not null default now()
);
...
-- ---------------------------------------------------------------------------
-- 12) ฟังก์ชันออกเลขที่เอกสารอัตโนมัติ (running number ต่อเดือน)
--     รูปแบบ: <PREFIX>-<YYYYMM>-<NNNN> เช่น PR-202606-0001
--     ใช้ advisory lock กันการชนกันเมื่อมีหลาย transaction พร้อมกัน
-- ---------------------------------------------------------------------------
...
declare
  ...
  next_seq  integer;
begin
  -- ล็อกเฉพาะ prefix+เดือนนี้ เพื่อกันเลขซ้ำในสถานการณ์ concurrent
...
-- ============================================================================
-- 13) Row Level Security (RLS) — สำคัญมากด้านความปลอดภัย
--     เปิด RLS ทุกตาราง แล้วกำหนด policy ให้ผู้ใช้ที่ล็อกอินเท่านั้นเข้าถึงได้
--     และจำกัดการแก้ไขตามบทบาท (role)
-- ============================================================================
...
-- profiles: ทุกคนที่ล็อกอินอ่านได้, แก้ไขได้เฉพาะของตัวเองหรือ admin
...
-- ข้อมูลหลัก (master data): อ่านได้ทุกคนที่ล็อกอิน, แก้ไขเฉพาะ admin/purchaser
...
-- PR: ทุกคนที่ล็อกอินอ่านได้; ผู้ขอสร้าง/แก้ของตัวเองได้, admin/manager จัดการได้ทั้งหมด
...
-- PO: อ่านได้ทุกคน; สร้าง/แก้ได้เฉพาะ purchaser/admin
...
-- approvals: อ่านได้ทุกคน; ผู้อนุมัติบันทึกผลของตัวเองได้, admin จัดการได้
...
-- goods receipts: อ่านได้ทุกคน; สร้าง/แก้ได้เฉพาะ purchaser/admin
...
-- audit_logs: อ่านได้เฉพาะ admin/manager; เขียนผ่าน server (service role) เท่านั้น
